...
Na trgu že obstajajo rešitve za ocenjevanje vrednosti vozil, vendar so mnoge plačljive, prilagojene tujim trgom ali pa temeljijo na zastarelih podatkih. Zaradi tega njihove ocene
 pogosto ne odražajo dejanskega stanja na slovenskem trgu rabljenih vozil. Kar lahko vodi do istih problemov, kot so omenjeni zgoraj. \\
...
Spletne aplikacije so sestavljene iz uporabniškega vmesnika (odjemalca) in strežniškega dela aplikacije (zalednega sistema). Uporabniški vmesnik uporabniku omogoča prikaz podatkov ter interakcijo z aplikacijo. 
Njegova glavna naloga je zagotavljanje preglednega in uporabniku prijaznega dostopa do funkcionalnosti sistema. Poleg prikaza podatkov skrbi tudi za zajem uporabniških vnosov, komunikacijo s strežniškim delom aplikacije ter lokalno shranjevanje določenih podatkov v spletnem brskalniku.
Sodobni uporabniški vmesniki so pogosto sestavljeni iz komponent. Komponenta predstavlja samostojen del uporabniškega vmesnika, ki ga je mogoče večkrat uporabiti na različnih mestih znotraj aplikacije. 
Takšen pristop omogoča boljšo organizacijo kode, lažje vzdrževanje in večjo možnost ponovne uporabe funkcionalnosti. \\

Strežniški del aplikacije je odgovoren za obdelavo zahtevkov, ki jih prejme od odjemalca, ter za vračanje ustreznih odgovorov. Poleg tega skrbi za izvajanje poslovne logike, preverjanje uporabniških pravic, avtentikacijo in avtorizacijo uporabnikov ter komunikacijo s podatkovno bazo.
V večini sodobnih spletnih aplikacij strežniški del omogoča dostop do podatkov preko programskih vmesnikov (API). Eden izmed najbolj razširjenih pristopov je arhitekturni slog REST, ki omogoča standardizirano komunikacijo med različnimi sistemi. 
Strežniški del aplikacije lahko vključuje tudi dodatne mehanizme, kot so omejevanje števila zahtevkov (angl. \textit{rate limiting}), predpomnjenje podatkov in beleženje dogodkov.
Podatki spletne aplikacije se običajno shranjujejo v podatkovni bazi, ki omogoča trajno hranjenje informacij ter učinkovito poizvedovanje po njih. Strežniški del aplikacije tako predstavlja povezavo med uporabniškim vmesnikom in podatkovno bazo ter skrbi za varen in nadzorovan dostop do podatkov.
